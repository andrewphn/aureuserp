...
class ConstructionStandardsService
{
    ...
    private const FALLBACK_DEFAULTS = [
        ...
        'face_frame_thickness' => 1.0,  // 5/4 hardwood = 1" actual (TCS standard)
        'face_frame_min_rail_gap' => 0.75,  // TCS Standard: Rails only added if gap >= 3/4"
        // Material Thickness
        ...
        'finished_end_gap' => 0.25,              // 1/4" gap between panel and cabinet side
        ...
        'stretchers_on_top' => true,         // Stretchers sit ON TOP of sides = FULL WIDTH (TCS standard)
        'stiles_to_floor' => true,           // Face frame stiles extend to FLOOR (covers end panels) (TCS standard)
    ...
    /**
     * Get minimum gap size that requires a rail to cover it.
     * TCS Rule: If gap between components >= 3/4", add a rail.
     * Rails are only added to compensate for gaps that exceed this threshold.
     */
    public function getMinRailGap(Cabinet $cabinet): float
    {
        return $this->resolveTemplate($cabinet)->face_frame_min_rail_gap ?? 0.75;
    }

    /**
     * Check if a gap requires a rail to cover it.
     * TCS Rule: Rails only added when gap >= min_rail_gap (3/4")
     *
     * @param Cabinet $cabinet
     * @param float $gapSize Size of the gap in inches
     * @return bool True if a rail should be added
     */
    public function needsRailForGap(Cabinet $cabinet, float $gapSize): bool
    {
        return $gapSize >= $this->getMinRailGap($cabinet);
    }

    /**
     * Get face frame thickness for a cabinet.
     * Checks cabinet's selected face frame material product first,
     * then falls back to construction template default.
     */
    public function getFaceFrameThickness(Cabinet $cabinet): float
    {
        // First check if cabinet has a face frame material product with thickness
        if ($cabinet->face_frame_material_product_id) {
            $product = $cabinet->faceFrameMaterialProduct;
            if ($product) {
                // Try to get thickness from product attributes
                $thickness = $product->getSpecValue('Thickness');
        ...
        // Fall back to construction template default
    ...
    /**
     * Check if finished end panels are enabled.
     * TCS Standard: true (exposed sides get end panels)
     */
    public function finishedEndEnabled(Cabinet $cabinet): bool
    {
        return $this->resolveTemplate($cabinet)->finished_end_enabled ?? true;
    }
    ...
    /**
     * Check if sides sit on top of bottom panel.
     * TCS Standard: true (sides rest on bottom, not beside it)
     */
    public function sidesOnBottom(Cabinet $cabinet): bool
    {
        return $this->resolveTemplate($cabinet)->sides_on_bottom ?? true;
    }

    /**
     * Check if back is inset from sides.
     * false = back fits between sides (TCS standard)
     * true = back sits behind sides
     */
    public function backInsetFromSides(Cabinet $cabinet): bool
    {
        return $this->resolveTemplate($cabinet)->back_inset_from_sides ?? false;
    }

    /**
     * Check if stretchers fit between sides.
     */
    public function stretchersBetweenSides(Cabinet $cabinet): bool
    {
        return $this->resolveTemplate($cabinet)->stretchers_between_sides ?? true;
    }

    /**
     * Check if stretchers sit on top of sides.
     * TCS Standard: true (stretchers rest on top of sides)
     */
    public function stretchersOnTop(Cabinet $cabinet): bool
    {
        return $this->resolveTemplate($cabinet)->stretchers_on_top ?? true;
    }

    /**
     * Check if face frame stiles extend to floor.
     * TCS Standard: true (stiles go full cabinet height to cover end panels)
     */
    public function stilesToFloor(Cabinet $cabinet): bool
    {
        return $this->resolveTemplate($cabinet)->stiles_to_floor ?? true;
    }

    /**
     * Calculate effective side panel height based on assembly rules.
     *
     * @param Cabinet $cabinet
     * @param float $boxHeight Total box height
     * @return float Side panel height
     */
    public function calculateSidePanelHeight(Cabinet $cabinet, float $boxHeight): float
    {
        if ($this->sidesOnBottom($cabinet)) {
            // Sides sit ON TOP of bottom, so subtract bottom thickness
            return $boxHeight - $this->getBoxMaterialThickness($cabinet);
        }
        // Sides extend full height
        return $boxHeight;
    }

    /**
     * Calculate side panel Y position based on assembly rules.
     *
     * @param Cabinet $cabinet
     * @return float Y position (0 if full height, bottom_thickness if sitting on bottom)
     */
    public function getSidePanelYPosition(Cabinet $cabinet): float
    {
        if ($this->sidesOnBottom($cabinet)) {
            return $this->getBoxMaterialThickness($cabinet);
        }
        return 0.0;
    }

    /**
     * Calculate bottom panel width based on assembly rules.
     *
     * @param Cabinet $cabinet
     * @param float $cabinetWidth Total cabinet width
     * @return float Bottom panel width
     */
    public function calculateBottomPanelWidth(Cabinet $cabinet, float $cabinetWidth): float
    {
        if ($this->sidesOnBottom($cabinet)) {
            // Bottom is full width (sides sit on top)
            return $cabinetWidth;
        }
        // Bottom fits between sides
        return $cabinetWidth - (2 * $this->getSidePanelThickness($cabinet));
    }

    /**
     * Get bottom panel X position based on assembly rules.
     *
     * @param Cabinet $cabinet
     * @return float X position (0 if full width, side_thickness if between sides)
     */
    public function getBottomPanelXPosition(Cabinet $cabinet): float
    {
        if ($this->sidesOnBottom($cabinet)) {
