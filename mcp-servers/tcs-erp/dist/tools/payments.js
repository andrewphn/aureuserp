...
export async function handlePaymentTool(client, toolName, args) {
    switch (toolName) {
        case 'list_payments':
            return client.listPayments(args);
        case 'get_payment':
            return client.getPayment(args.id);
        case 'create_payment':
            return client.createPayment(args);
        case 'update_payment': {
            const { id, ...data } = args;
            return client.updatePayment(id, data);
        }
        case 'delete_payment':
            return client.deletePayment(args.id);
        case 'post_payment':
            return client.postPayment(args.id);
        case 'cancel_payment':
            return client.cancelPayment(args.id, args.reason);
