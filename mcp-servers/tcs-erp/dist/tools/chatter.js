...
export async function handleChatterTool(client, toolName, args) {
    switch (toolName) {
        case 'list_chatter':
            return client.listChatter(args);
        case 'get_chatter':
            return client.getChatter(args.id, args.include);
        case 'create_chatter':
            return client.createChatter(args);
        case 'update_chatter': {
            const { id, ...data } = args;
            return client.updateChatter(id, data);
        }
        case 'delete_chatter':
            return client.deleteChatter(args.id);
        case 'get_chatter_for_resource':
            return client.getChatterForResource(args.type, args.id, args);
        case 'add_chatter_to_resource':
            return client.addChatterToResource(args.type, args.id, args);
        case 'pin_chatter':
            return client.pinChatter(args.id);
        case 'unpin_chatter':
            return client.unpinChatter(args.id);
