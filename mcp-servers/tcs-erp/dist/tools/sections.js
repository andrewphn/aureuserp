...
export async function handleSectionTool(client, toolName, args) {
    switch (toolName) {
        case 'list_sections':
            return client.listSections(args.cabinet_id);
        case 'get_section':
            return client.getSection(args.id);
        case 'create_section': {
            const { cabinet_id, ...data } = args;
            return client.createSection(cabinet_id, data);
        }
        case 'update_section': {
            const { id, ...data } = args;
            return client.updateSection(id, data);
        }
        case 'delete_section':
            return client.deleteSection(args.id);
        default:
