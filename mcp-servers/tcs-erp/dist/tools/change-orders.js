...
export async function handleChangeOrderTool(client, toolName, args) {
    switch (toolName) {
        case 'list_change_orders':
            return client.listChangeOrders(args);
        case 'get_change_order':
            return client.getChangeOrder(args.id);
        case 'create_change_order':
            return client.createChangeOrder(args);
        case 'update_change_order': {
            const { id, ...data } = args;
            return client.updateChangeOrder(id, data);
        }
        case 'delete_change_order':
            return client.deleteChangeOrder(args.id);
        case 'approve_change_order':
            return client.approveChangeOrder(args.id, args);
        case 'reject_change_order':
            return client.rejectChangeOrder(args.id, args.reason);
        case 'get_change_orders_by_project':
            return client.getChangeOrdersByProject(args.project_id);
        default:
