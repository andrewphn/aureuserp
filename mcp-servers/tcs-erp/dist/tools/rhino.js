...
export async function handleRhinoTool(client, toolName, args) {
    switch (toolName) {
        case 'get_interpretation_context':
            return client.getInterpretationContext(args.review_id);
        case 'save_interpretation':
            return client.saveInterpretation(args.review_id, args.interpretation);
        case 'rhino_get_document_info':
            return client.rhinoGetDocumentInfo();
        case 'rhino_list_groups':
            return client.rhinoListGroups(args.filter);
        case 'rhino_extract_cabinet':
            return client.rhinoExtractCabinet(args.group_name, args.include_dimensions);
        case 'rhino_extract_all':
            return client.rhinoExtractAll(args.project_id, args.include_dimensions);
        case 'rhino_trigger_extraction':
            return client.rhinoTriggerExtraction(args.project_id, args.options);
        case 'rhino_get_extraction_status':
            return client.rhinoGetExtractionStatus(args.job_id);
        case 'rhino_sync_to_rhino':
            return client.rhinoSyncToRhino(args.cabinet_id, args.create_if_missing);
        case 'rhino_sync_from_rhino':
            return client.rhinoSyncFromRhino(args.group_name, args.cabinet_id, args.force);
        case 'rhino_execute_script':
            return client.rhinoExecuteScript(args.script, args.timeout);
        default:
