...
export async function handleSalesOrderTool(client, toolName, args) {
    switch (toolName) {
        case 'list_sales_orders':
            return client.listSalesOrders(args);
        case 'get_sales_order':
            return client.getSalesOrder(args.id, args.include);
        ...
        case 'delete_sales_order':
            return client.deleteSalesOrder(args.id);
        case 'confirm_sales_order':
            return client.confirmSalesOrder(args.id);
        case 'cancel_sales_order':
            return client.cancelSalesOrder(args.id, args.reason);
        case 'create_sales_order_invoice':
            return client.createSalesOrderInvoice(args.id, args);
        case 'send_sales_order_email':
            return client.sendSalesOrderEmail(args.id, args);
        case 'list_sales_order_lines':
            return client.listSalesOrderLines(args.order_id);
        case 'create_sales_order_line': {
            const { order_id, ...data } = args;
            return client.createSalesOrderLine(order_id, data);
        }
        case 'update_sales_order_line': {
            const { id, ...data } = args;
            return client.updateSalesOrderLine(id, data);
        }
        case 'delete_sales_order_line':
            return client.deleteSalesOrderLine(args.id);
        default:
