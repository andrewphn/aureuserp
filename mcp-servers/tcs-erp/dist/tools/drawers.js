...
export async function handleDrawerTool(client, toolName, args) {
    switch (toolName) {
        case 'list_drawers':
            return client.listDrawers(args.section_id, args.cabinet_id);
        case 'get_drawer':
            return client.getDrawer(args.id);
        case 'create_drawer': {
            const { section_id, ...data } = args;
            return client.createDrawer(section_id, data);
        }
        case 'update_drawer': {
            const { id, ...data } = args;
            return client.updateDrawer(id, data);
        }
        case 'delete_drawer':
            return client.deleteDrawer(args.id);
        default:
