...
export async function handleDoorTool(
  ...
  switch (toolName) {
    case 'list_doors':
      return client.listDoors(args.section_id as number | undefined, args.cabinet_id as number | undefined);
    case 'get_door':
      return client.getDoor(args.id as number);
    case 'create_door': {
      const { section_id, ...data } = args;
      return client.createDoor(section_id as number, data);
    }
    case 'update_door': {
      const { id, ...data } = args;
      return client.updateDoor(id as number, data);
    }
    case 'delete_door':
      return client.deleteDoor(args.id as number);
    default:
