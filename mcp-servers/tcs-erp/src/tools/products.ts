...
export async function handleProductTool(
  ...
  switch (toolName) {
    ...
    case 'get_product':
      return client.getProduct(args.id as number);
    case 'create_product':
      return client.createProduct(args);
    case 'update_product': {
      const { id, ...data } = args;
      return client.updateProduct(id as number, data);
    }
    case 'delete_product':
      return client.deleteProduct(args.id as number);
    case 'search_products':
      return client.searchProducts(args.query as string, args.limit as number | undefined);
    default:
