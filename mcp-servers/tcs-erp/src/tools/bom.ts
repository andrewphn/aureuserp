...
export async function handleBomTool(
  ...
  switch (toolName) {
    ...
    case 'get_bom':
      return client.getBom(args.id as number);
    case 'create_bom':
      return client.createBom(args);
    case 'update_bom': {
      const { id, ...data } = args;
      return client.updateBom(id as number, data);
    }
    case 'delete_bom':
      return client.deleteBom(args.id as number);
    case 'get_bom_by_project':
      return client.getBomByProject(args.project_id as number);
    case 'get_bom_by_cabinet':
      return client.getBomByCabinet(args.cabinet_id as number);
    case 'generate_bom':
      return client.generateBom(args.project_id as number, args.overwrite as boolean | undefined);
    case 'bulk_update_bom_status':
      return client.bulkUpdateBomStatus(args.ids as number[], args.status as string);
    default:
