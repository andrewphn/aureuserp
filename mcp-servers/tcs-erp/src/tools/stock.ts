...
export async function handleStockTool(
  ...
  switch (toolName) {
    ...
    case 'get_stock':
      return client.getStock(args.id as number);
    case 'get_stock_by_product':
      return client.getStockByProduct(args.product_id as number);
    case 'get_stock_by_location':
      return client.getStockByLocation(args.location_id as number);
    case 'adjust_stock':
