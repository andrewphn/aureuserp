...
export const cabinetTools: Tool[] = [
  {
    name: 'list_cabinets',
...
export async function handleCabinetTool(
  ...
  switch (toolName) {
    ...
    case 'get_cabinet':
      return client.getCabinet(args.id as number, args.include as string[] | undefined);
    case 'create_cabinet': {
      const { cabinet_run_id, ...data } = args;
      return client.createCabinet(cabinet_run_id as number, data);
    }
    case 'update_cabinet': {
      const { id, ...data } = args;
      return client.updateCabinet(id as number, data);
    }
    case 'delete_cabinet':
      return client.deleteCabinet(args.id as number);
    case 'calculate_cabinet':
      return client.calculateCabinet(args.id as number);
    case 'get_cabinet_cut_list':
      return client.getCabinetCutList(args.id as number, args.format as string | undefined);
    default:
