...
export async function handleInvoiceTool(
  ...
  switch (toolName) {
    ...
    case 'get_invoice':
      return client.getInvoice(args.id as number, args.include as string[] | undefined);
    case 'create_invoice':
      return client.createInvoice(args);
    case 'update_invoice': {
      const { id, ...data } = args;
      return client.updateInvoice(id as number, data);
    }
    case 'delete_invoice':
      return client.deleteInvoice(args.id as number);
    case 'post_invoice':
      return client.postInvoice(args.id as number);
    case 'pay_invoice':
      return client.payInvoice(args.id as number, args);
    case 'create_invoice_credit_note':
      return client.createInvoiceCreditNote(args.id as number, args);
    case 'reset_invoice_to_draft':
      return client.resetInvoiceToDraft(args.id as number);
    case 'send_invoice_email':
      return client.sendInvoiceEmail(args.id as number, args);
    // Bills
    case 'list_bills':
      return client.listBills(args);
    case 'get_bill':
      return client.getBill(args.id as number, args.include as string[] | undefined);
    case 'create_bill':
      return client.createBill(args);
    case 'update_bill': {
      const { id, ...data } = args;
      return client.updateBill(id as number, data);
    }
    case 'delete_bill':
      return client.deleteBill(args.id as number);
    case 'post_bill':
      return client.postBill(args.id as number);
    case 'pay_bill':
      return client.payBill(args.id as number, args);
    case 'reset_bill_to_draft':
      return client.resetBillToDraft(args.id as number);
    default:
