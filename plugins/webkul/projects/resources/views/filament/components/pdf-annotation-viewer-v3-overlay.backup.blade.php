        ...
        <div class="flex items-center gap-4 flex-wrap">
            ...
            <div class="flex items-center gap-2">
                <h2 class="text-lg font-semibold text-white flex items-center gap-2">
                    ✨ V3 Annotation System - Page {{ $pageNumber }}
                    <span class="px-2 py-1 text-xs rounded font-semibold bg-blue-600">PDFObject + Overlay</span>
                </h2>
            </div>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('annotationSystemV3', (config) => ({
                ...
                async extractPdfDimensions() {
                    ...
                    console.log(`✓ Page dimensions: ${Math.round(this.pageDimensions.width)} × ${Math.round(this.pageDimensions.height)} pts`);
                ...
                async displayPdf() {
                    console.log('📄 Displaying PDF with PDFObject.js...');

                    // Wait for PDFObject to be available
                    await this.waitForPDFObject();
                    ...
                    if (success) {
                        ...
                        throw new Error('PDFObject.js failed to embed PDF');
                    }
                },

                // Wait for PDFObject to be available
                async waitForPDFObject() {
                    let attempts = 0;
                    while (typeof PDFObject === 'undefined' && attempts < 50) {
                        await new Promise(resolve => setTimeout(resolve, 100));
                        attempts++;
                    }
                    if (typeof PDFObject === 'undefined') {
                        throw new Error('PDFObject.js failed to load');
                    }
                    console.log('✓ PDFObject.js ready');
                },
                ...
                pdfToScreen(pdfX, pdfY, width = 0, height = 0) {
                    ...
                    const screenY = normalizedY * rect.height;
