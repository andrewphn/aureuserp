...
>
    ...
    <template x-if="selectedRoom">
        <span class="flex items-center gap-1">
            ...
            <button
                @click="clearToRoom()"
                :class="!selectedLocation ? 'font-medium text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400'"
    ...
    <template x-if="selectedLocation">
        <span class="flex items-center gap-1">
            ...
            <button
                ...
                :class="!selectedRun ? 'font-medium text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400'"
                class="transition-colors"
    ...
    <template x-if="selectedRun">
        <span class="flex items-center gap-1">
            <x-heroicon-m-chevron-right class="w-4 h-4 text-gray-400" />
            <span class="font-medium text-gray-900 dark:text-white" x-text="selectedRun?.name || 'Untitled'"></span>
        </span>
    </template>
</nav>
