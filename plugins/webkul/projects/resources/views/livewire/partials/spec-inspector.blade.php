...
<div class="space-y-4">
        ...
        <div class="space-y-4">
            ...
            >
                ...
                <div class="flex items-center gap-6">
                    <div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">Cabinets</div>
                        <div class="text-sm font-semibold text-gray-900 dark:text-white" x-text="(selectedRun.children || []).length"></div>
                    </div>
                    <div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">Linear Feet</div>
                        <div class="text-sm font-semibold tabular-nums text-primary-600 dark:text-primary-400" x-text="formatLinearFeet(selectedRun.linear_feet || 0)"></div>
                    </div>
                </div>
            </x-filament::section>
            ...
            >
                ...
                <div class="space-y-3">
                    ...
                    {{-- Material Category --}}
                    <div class="grid grid-cols-3 items-center gap-3">
                        <label class="text-sm font-medium text-gray-700 dark:text-gray-300">Material</label>
                        <div class="col-span-2">
                            <x-filament::input.wrapper>
                                <x-filament::input.select
                    ...
                    {{-- Finish Option --}}
                    <div class="grid grid-cols-3 items-center gap-3">
                        <label class="text-sm font-medium text-gray-700 dark:text-gray-300">Finish</label>
                        <div class="col-span-2">
                            <x-filament::input.wrapper>
                                <x-filament::input.select
            ...
            <x-filament::section compact>
                <x-slot name="heading">Cabinets</x-slot>
                <x-slot name="headerEnd">
                    <x-filament::button
                        size="xs"
                        color="gray"
    ...
    <template x-if="selectedLocation && !selectedRun">
        <div class="space-y-4">
            ...
            >
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div>
                        ...
                        <div class="text-sm font-semibold text-gray-900 dark:text-white" x-text="(selectedLocation.children || []).length"></div>
                    </div>
                    <div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">Linear Feet</div>
                        <div class="text-sm font-semibold tabular-nums text-primary-600 dark:text-primary-400" x-text="formatLinearFeet(selectedLocation.linear_feet || 0)"></div>
                    </div>
                    <div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">Estimated</div>
                        <div class="text-sm font-semibold tabular-nums text-success-600 dark:text-success-400">$<span x-text="(selectedLocation.estimated_price || 0).toLocaleString('en-US', {minimumFractionDigits: 2})"></span></div>
                    </div>
                </div>
            </x-filament::section>
            ...
            {{-- Runs List Section --}}
            <x-filament::section compact>
                <x-slot name="heading">Cabinet Runs</x-slot>
                <x-slot name="headerEnd">
                    <x-filament::button
                        size="xs"
                        color="gray"
                ...
                <div class="-mx-4 -mb-4 divide-y divide-gray-200 dark:divide-gray-700">
                    <template x-if="!(selectedLocation.children || []).length">
                        <div class="px-4 py-8 text-center">
                            <div class="w-12 h-12 mx-auto mb-3 rounded-full flex items-center justify-center bg-gray-100 dark:bg-gray-800">
                    ...
                    <template x-for="(run, runIdx) in (selectedLocation.children || [])" :key="run.id || runIdx">
                        <div
                            ...
                            class="flex items-center justify-between px-4 py-3 cursor-pointer group transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50"
                        >
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <x-heroicon-m-chevron-right class="w-5 h-5 text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                            </div>
                        </div>
                    </template>
                </div>
    ...
    <template x-if="selectedRoom && !selectedLocation">
        <div class="space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div class="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3">
                    <div class="flex items-center justify-between mb-2">
                        <div class="flex items-center gap-2">
                            <div class="p-1.5 rounded-md bg-primary-50 dark:bg-primary-900/30">
                                <x-heroicon-s-home class="w-4 h-4 text-primary-600 dark:text-primary-400" />
                            </div>
                            <span class="font-semibold text-gray-900 dark:text-white" x-text="selectedRoom.name || 'Room'"></span>
                        </div>
                ...
                <div class="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3">
                    <div class="flex items-center justify-between mb-2">
                        <div class="flex items-center gap-2">
                            <div class="p-1.5 rounded-md bg-warning-50 dark:bg-warning-900/30">
                                <x-heroicon-s-currency-dollar class="w-4 h-4 text-warning-600 dark:text-warning-400" />
                            </div>
                            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Pricing</span>
                        </div>
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <select
                            ...
                            class="text-xs py-1.5 px-2 rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-primary-500 focus:border-primary-500"
                        >
                            <template x-for="(label, key) in pricingTiers" :key="key">
            ...
            <x-filament::section compact>
                <x-slot name="heading">Wall Locations</x-slot>
                <x-slot name="headerEnd">
                    <x-filament::button
                        size="xs"
                        color="gray"
    ...
    <template x-if="!selectedRoom">
        <div class="flex flex-col items-center justify-center h-full py-16 text-center">
            ...
            <h3 class="text-base font-semibold mb-1 text-gray-900 dark:text-white">Select a room to begin</h3>
